...
def validate(model, VT_star, N_star, save=False):
    ...
    for task_id in range(model.num_learned):
        vt = VT_star[task_id]
        n_exact = N_star[task_id]

        model.set_task(task_id)

        n_pred = model.predict(vt)

        error_u_relative, error_u_abs, error_u_linf = error(n_pred, n_exact)
        error_u_relative_list.append(error_u_relative)
        error_u_abs_list.append(error_u_abs)
        error_u_linf_list.append(error_u_linf)

    print('Error N rel: ', error_u_relative_list)
    print('Error N abs: ', error_u_abs_list)
    print('Error N linf: ', error_u_linf_list)

    if save:
        if model.num_learned > 1:
            import os
            if os.path.exists(f'{file_name}.pickle'):
